...
<aos:html title="借款端，放款资金流水报表" base="http" lib="ext">
<aos:body>
</aos:body>
</aos:html>
...
<aos:onready>
	<aos:viewport layout="border" id="_g_stock">
		...
		<aos:formpanel layout="column" columnWidth="1" region="north"
			...
			<aos:datefield fieldLabel="资金到账日期" name="arrived_start_time_" emptyText="开始日期" columnWidth="0.188" editable="false"/>
			<aos:displayfield value="-" width="20" margin="0 0 0 10" />
			<aos:datefield fieldLabel="" name="arrived_end_time_" emptyText="结束日期" columnWidth="0.118" editable="false"/>
			...
			<aos:datefield fieldLabel="流水登账日期" name="record_start_time_" emptyText="开始日期" columnWidth="0.188" editable="false" value="${currentDate}"/>
			<aos:displayfield value="-" width="20" margin="0 0 0 10" />
			<aos:datefield fieldLabel="" name="record_end_time_" emptyText="结束日期" columnWidth="0.118" editable="false" value="${currentDate}"/>
		...
		<aos:gridpanel id="_g_loan" url="releaseBusinessService.releaseList"
			onrender="_g_loan_query" pageSize="30" autoScroll="true" border="true"  forceFit="false"
			region="center">
			
			<aos:docked forceBoder="1 0 1 0">
				<aos:dockeditem xtype="tbtext" text="放款资金流水列表" />
				<aos:dockeditem xtype="tbseparator" />
				<aos:dockeditem text="导出Excel报表" icon="icon70.png" onclick="fn_xls" />
			</aos:docked>

			<aos:column type="rowno" />
			<aos:column dataIndex="contract_id_" hidden="true"/>
			<aos:column header="放款资金流水ID" dataIndex="id_" align="center" width="140"/>
			<aos:column header="归属业务线" dataIndex="platform_id_" align="center" width="140"/>
			<aos:column header="业务线内借款合同编号" dataIndex="contract_code_" align="center" width="140"/>
			...
			<aos:column header="借款人收款帐号" dataIndex="target_account_" align="center" width="140"/>
			<aos:column header="交易金额(放款金额)" dataIndex="trans_amount_" align="center" width="140" type="number" format="0,000.00" />
	...
	<script type="text/javascript">
		...
		function fn_xls() {
			...
			AOS.file('do.jhtml?router=releaseBusinessService.exprotExcel&juid=${juid}');
		}
		...
		function fn_button_cut_render(value, metaData, record, rowIndex, colIndex, store) {
			return '<input type="button" value="截留科目" class="cbtn" onclick="_w_staging_show();" />';
		}
		
		//资金明细窗口
		function _w_staging_onshow() {
			var record = AOS.selectone(_g_loan);
			_w_staging.setTitle('借款合同 ' + record.data.contract_code_ + '的截留科目明细信息');
			//获取动态表格元素
			AOS.ajax({
				url : 'releaseBusinessService.cutDynamicFiled',
				params : {
