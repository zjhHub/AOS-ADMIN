...
<aos:onready>
		...
		<aos:hiddenfield name="juid" id="juid" value="${juid}" />
			...
			<aos:formpanel id="_f_trans" layout="column" columnWidth="1" border="false" rowSpace="30" >
				<aos:docked forceBoder="0 1 1 0">
					<aos:dockeditem xtype="tbtext" text="增加余额操作" />
				</aos:docked>
			
				<aos:combobox name="position_" fieldLabel="增加到目标位置" columnWidth="0.60" allowBlank="false" dicField="position_" />
				<aos:textfield name="amount_" fieldLabel="增加余额数" columnWidth="0.60" allowBlank="false" regex="/^\d*\.{0,2}\d{0,2}$/" emptyText="请输入数字和小数点"/>
				...
				<aos:docked dock="bottom" ui="footer" margin="0 0 8 0" >
					...
					<aos:dockeditem text="增加余额" icon="add.png" onclick="_g_add" />
	...
	<script type="text/javascript">
		   ...
		   var form = _f_trans.getForm();	       
	        ...
	        form.submit({
	            ...
	            success: function (form, action) {
	                ...
	                window.location.href = 'do.jhtml?router=accountsService.balanceDetail&juid=${juid}&id_=${id_}&plus_minus_id=1';
	            }
	        }); 
		} 
		...
		//短信验证
		function bn_sms() {
			var msg_code_ = AOS.getValue('msg_code_');
			 AOS.ajax({
	 	        	params : {
	 	        		msg_code_ : msg_code_
	 	        	},
	 				url:'smsBalanceService.smsValidate',
		            ok: function (data) {
		            	AOS.tip(data.appmsg);
		            }
		        });
