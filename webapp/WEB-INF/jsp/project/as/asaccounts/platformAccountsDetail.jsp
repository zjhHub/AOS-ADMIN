...
<aos:html title="业务线账户详情" base="http" lib="ext">
<aos:body>
</aos:body>
</aos:html>
<aos:onready>
	<aos:viewport layout="border">
		...
		<aos:formpanel id="pri_info" region="north">
			 	<aos:fieldset title="业务线账户信息摘要" labelWidth="90" columnWidth="1">
					<aos:textfield name="account_code_" value="${po['account_code_']}" fieldLabel="账户编码" readOnly="true" columnWidth="0.25" />
					<aos:textfield name="purpose_" value="${po['purpose_']}" fieldLabel="账户用途" readOnly="true" columnWidth="0.25" />
					<aos:textfield name="name_" value="${po['name_']}" fieldLabel="归属业务线" readOnly="true" columnWidth="0.25" />
					<aos:textfield name="create_time_" value="${po['create_time_']}" fieldLabel="创建时间" readOnly="true" columnWidth="0.25" />
					<aos:textfield name="total_balance_" value="${po['total_balance_']}" fieldLabel="账面总余额" readOnly="true" columnWidth="0.25" />
					<aos:textfield name="available_balance_" value="${po['available_balance_']}" fieldLabel="可用余额" readOnly="true" columnWidth="0.25" />
					<aos:textfield name="froze_balance_" value="${po['froze_balance_']}" fieldLabel="冻结余额" readOnly="true" columnWidth="0.25" />
				</aos:fieldset>
		</aos:formpanel>
		...
		<aos:tabpanel id="id_tabpanel" region="center"  tabBarHeight="30">
			...
			<aos:tab title="余额手动增减记录" id="_tab_balanplus" onactivate="_g_plus_query" layout="border">
				<aos:gridpanel id="_g_plus" url="accountsService.balancePlusMinus" pageSize="20" region="center">
					<aos:column type="rowno" />
					<aos:column header="下载凭证" dataIndex="certificate_path_" hidden="true" />
					<aos:column header="增减标记" dataIndex="tag_" hidden="true" />
					<aos:column header="手动增减ID" dataIndex="id_" width="30" />
					<aos:column header="发生时间" dataIndex="created_time_" width="50" />
					<aos:column header="增加" dataIndex="zjamount" width="30" />
					<aos:column header="减少" dataIndex="jsamount" width="30" />
					<aos:column header="增减位置" dataIndex="position_" rendererField="move_record_type_" width="40" />
					<aos:column header="财务备注" dataIndex="remark_" width="50" />
					<aos:column header="负责人" dataIndex="created_by" width="30" />
					<aos:column header="操作" align="center" fixedWidth="130" rendererFn="fn_bt_plusDedail"/>
				</aos:gridpanel>
			</aos:tab>
	...
	<script type="text/javascript">
		...
		//余额手动增减记录
		function _g_plus_query() {
			if (!_tab_balanplus.isVisible()) {
				_tabpanel.setActiveTab(_tab_balanplus);
			}
			var accountID = id_.getValue();
			var params = {
					id_ : accountID
			};
			_g_plus_store.getProxy().extraParams = params;
			_g_plus_store.loadPage(1);
		}
		...
		//余额手动增减记录 
		function fn_bt_plusDedail(value, metaData, record, rowIndex, colIndex, store) {
			return '<input type="button" value="下载凭证" class="cbtn" onclick="_btn_downonclick();" />  <input type="button" value="账户快照" class="cbtn" onclick="_btn_plusonclick();" />';
		}
...
<script type="text/javascript">
	...
	function _btn_inoutonclick(){
		...
		parent.fnaddtab('balanceTransService.companyBalanceSnapshotDetail&snapshot_id_=' + snapshot_id_, '快照：'+ snapshot_id_+'的详情', '998'+snapshot_id_);
	}
	...
	function _btn_insideonclick(){
		...
		var snapshot_id_ = record.data.snapshot_id_;
		parent.fnaddtab('balanceTransService.companyBalanceSnapshotDetail&snapshot_id_=' + snapshot_id_, '快照：'+ snapshot_id_+'的详情', '977'+snapshot_id_);
	}
	
	//余额手动增减记录--账户快照
	function _btn_plusonclick(){
		var record = AOS.selectone(Ext.getCmp('_g_plus'));
		var tag = record.data.tag_;
		var plusId = record.data.id_;
		if(tag==1){
			parent.fnaddtab('balanceTransService.companyBalanceSnapshotDetail&origin_id_=' + plusId +'&reason_='+7, '快照详情', '976'+plusId);
		}else{
			parent.fnaddtab('balanceTransService.companyBalanceSnapshotDetail&origin_id_=' + plusId +'&reason_='+8, '快照详情', '975'+plusId);
		}
	}

	//下载凭证
	function _btn_downonclick() {
		var record = AOS.selectone(Ext.getCmp('_g_plus'));
		var certificate_path = record.data.certificate_path_;
		if(certificate_path == null || certificate_path == "" ){
			AOS.tip('没有凭证可以下载');
           return;
		}
		window.open(certificate_path);
	}
